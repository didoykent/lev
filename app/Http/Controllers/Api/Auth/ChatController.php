...
public function getUnreadMessages(){
  ...
  for($i=0; $i<$arrayCount; $i++){
    ...
    $messages = count(Message::where('tutors_id', $authuser->id)->where('student_id', $friendslists[$i]['id'])->where('read', 0)->get());
    ...
    for($i=0; $i<$arrayCount; $i++){
      ...
      $messages = count(Message::where('student_id', $authuser->id)->where('tutors_id', $friendslists[$i]['id'])->where('read', 0)->get());
        array_push($allUnread, $messages);
    }
  }

  return response()->json($allUnread);
