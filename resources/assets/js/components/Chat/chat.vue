...
var messageCount = 0;
console.log('clients sa room', data.clientsData)
console.log('client ko sbong', data.friend)
...
for( var i=0; i<data.clientsData.length; i++){
  ...
  axios.put(`/api/editMessage/${myData}`).then(function(response){
    ...
    for(var i=0; i<vm.myFriends.length; i++){
      ...
      if(vm.myFriends[i]['id'] === data.friend['id']){

      vm.myFriends[i]['notif']+=1
      }
    }
  })
...
vm.$socket.emit('latestMessage', {message: vm.message, friend: vm.getUserSock, messagedata: response.data, bonusdata: vm.$socket.id})
...
vm.$socket.emit('friendOnline', vm.currentUserId)
      ...
      await axios.get('api/getUnreadMessages').then(function(response){
